...
- **Product definition decorator** (`Elasticsearch/ProductElasticsearchDefinitionDecorator.php`) adds `.delimiter` sub-field to product name mapping.
- **Query boosting** (`Subscriber/ElasticsearchSearchSubscriber.php`): match_phrase (30x), match AND (20x), delimiter/wildcard (15x), prefix (1.1x).
